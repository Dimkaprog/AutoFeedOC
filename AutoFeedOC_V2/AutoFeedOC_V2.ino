...
const unsigned long interval_5min = 300000; //Как часто будет опрашиваться датчик DHT
...
void setup() {
  ...
  client.setInsecure(); 
  bot.setUpdateTime(1000); //Проверка сообщений
...
// ************** Dht data checker ***********************8
  if (currentTime - lastTime_5min >= interval_5min) {
    ...
    leftFood();
    if (hum >= 40 && !humWarned){
      bot.sendTo(atoll(CHAT_ID), "❗High humidity. Current: " + String(hum,0) + "%");
      humWarned = true;
    }
    else {
      if (hum < 38) {
    ...
    if (temp >= 33 && !tempWarned) {
      bot.sendTo(atoll(CHAT_ID), "❗High temperature. Current: " + String(temp,0) + "°C");
      tempWarned = true;
    }
     else {
      if (temp < 30) {
...
void catFeed() {
  ...
  if (foodPercent < 10 && !leftWarned) {
    bot.sendTo(atoll(CHAT_ID), "❗Small amount of feed. Left: " + String(foodPercent, 0) + "%");
    leftWarned = true;
  }
  else {
    if (foodPercent > 20) {
